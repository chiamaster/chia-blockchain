...
def create_harvester_service(
    ...
    farmer_peers: Optional[List[PeerInfo]],
...
async def async_main() -> int:
    ...
    if "farmer_peers" in service_config:
        for farmer in service_config["farmer_peers"]:
            peer_info = PeerInfo(
                str(get_host_addr(farmer["host"])), farmer["port"]
            )
            farmer_peers.append(peer_info)
    else:
        farmer_peer = PeerInfo(
            str(get_host_addr(service_config["farmer_peer"]["host"])),
            service_config["farmer_peer"]["port"],
        )
